...
export function ToDoListList() {
  ...
  return (
    <UIComponents.AdminLayout>
      <Ant.PageHeader
        title={t("management.to_do_list.list.header")}
        extra={[
          features.create ? (
            <Ant.Button
              key="1"
              onClick={() => router.go(Routes.TO_DO_LIST_CREATE)}
              icon={<PlusOutlined />}
            >
              {t("management.to_do_list.list.create_btn")}
            </Ant.Button>
          ) : null,
          <Ant.Button
            key="2"
      ...
      <Ant.Layout.Content>
        <Provider>
          <div className="page-to-do-list-list">
            {filtersOpened && (
